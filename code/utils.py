...
class Corpus(object):
    def __init__(self, path):
        ...
        if path.split('/')[-1] == 'bgl':
            ...
            self.valid_x, self.valid_y = self.vectorize(valid_seqs, 285)
            self.test_x, self.test_y = self.vectorize(test_seqs, 667)

        if path.split('/')[-1] == 'hdfs':
            self.valid_x, self.valid_y = self.vectorize(valid_seqs, 5051)
            self.test_x, self.test_y = self.vectorize(test_seqs, 11787)
